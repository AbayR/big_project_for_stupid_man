...
// update 
if(!empty($_GET['id']) && !empty($_GET['name']) && $_GET['name']=='home_photo')
{
  ...
  $pr1=$_FILES['pr1']['name'];
  $temppr1=$_FILES['pr1']['tmp_name'];  
  $pr1Path=$uploadTo.basename($pr1);
  $pr1Type= strtolower(pathinfo($pr1Path,PATHINFO_EXTENSION));

  $pr2=$_FILES['pr2']['name'];
  $temppr2=$_FILES['pr2']['tmp_name'];  
  $pr2Path=$uploadTo.basename($pr2);
  $pr2Type= strtolower(pathinfo($pr2Path,PATHINFO_EXTENSION));

  $pr3=$_FILES['pr3']['name'];
  $temppr3=$_FILES['pr3']['tmp_name'];  
  $pr3Path=$uploadTo.basename($pr3);
  $pr3Type= strtolower(pathinfo($pr3Path,PATHINFO_EXTENSION));

  $ft2=$_FILES['ft2']['name'];
  $tempft2=$_FILES['ft2']['tmp_name'];  
  $ft2Path=$uploadTo.basename($ft2);
  $ft2Type= strtolower(pathinfo($ft2Path,PATHINFO_EXTENSION));

  $ft3=$_FILES['ft3']['name'];
  $tempft3=$_FILES['ft3']['tmp_name'];  
  $ft3Path=$uploadTo.basename($ft3);
  $ft3Type= strtolower(pathinfo($ft3Path,PATHINFO_EXTENSION));

$ft4=$_FILES['ft4']['name'];
  $tempft4=$_FILES['ft4']['tmp_name'];  
  $ft4Path=$uploadTo.basename($ft4);
  $ft4Type= strtolower(pathinfo($ft4Path,PATHINFO_EXTENSION));

  $ft5=$_FILES['ft5']['name'];
  $tempft5=$_FILES['ft5']['tmp_name'];  
  $ft5Path=$uploadTo.basename($ft5);
  $ft5Type= strtolower(pathinfo($ft5Path,PATHINFO_EXTENSION));

  $ft6=$_FILES['ft6']['name'];
  $tempft6=$_FILES['ft6']['tmp_name'];  
  $ft6Path=$uploadTo.basename($ft6);
  $ft6Type= strtolower(pathinfo($ft6Path,PATHINFO_EXTENSION));

  $ft7=$_FILES['ft7']['name'];
  $tempft7=$_FILES['ft7']['tmp_name'];  
  $ft7Path=$uploadTo.basename($ft7);
  $ft7Type= strtolower(pathinfo($ft7Path,PATHINFO_EXTENSION));

  $ft8=$_FILES['ft8']['name'];
  $tempft8=$_FILES['ft8']['tmp_name'];  
  $ft8Path=$uploadTo.basename($ft8);
  $ft8Type= strtolower(pathinfo($ft8Path,PATHINFO_EXTENSION));

  $ft1=$_FILES['ft1']['name'];
  $tempft1=$_FILES['ft1']['tmp_name'];  
  $ft1Path=$uploadTo.basename($ft1);
  $ft1Type= strtolower(pathinfo($ft1Path,PATHINFO_EXTENSION));


  $allowedFileType=['jpg','jpeg','png','gif'];
  $uploadingError="";
// check allowed type file
  if(!in_array($pr1Type, $allowedFileType)){
  $uploadingError="Only 'jpg','jpeg','png','gif' formate of logo image are allowed";
}
 if(!in_array($pr2Type, $allowedFileType)){
  $uploadingError="Only 'jpg','jpeg','png','gif' formate of logo image are allowed";
}
 if(!in_array($pr3Type, $allowedFileType)){
  $uploadingError="Only 'jpg','jpeg','png','gif' formate of logo image are allowed";
}
 if(!in_array($ft1Type, $allowedFileType)){
  $uploadingError="Only 'jpg','jpeg','png','gif' formate of logo image are allowed";
}
 if(!in_array($ft2Type, $allowedFileType)){
  $uploadingError="Only 'jpg','jpeg','png','gif' formate of logo image are allowed";
}
 if(!in_array($ft3Type, $allowedFileType)){
  $uploadingError="Only 'jpg','jpeg','png','gif' formate of logo image are allowed";
}
 if(!in_array($ft4Type, $allowedFileType)){
  $uploadingError="Only 'jpg','jpeg','png','gif' formate of logo image are allowed";
}
 if(!in_array($ft5Type, $allowedFileType)){
  $uploadingError="Only 'jpg','jpeg','png','gif' formate of logo image are allowed";
}
 if(!in_array($ft6Type, $allowedFileType)){
  $uploadingError="Only 'jpg','jpeg','png','gif' formate of logo image are allowed";
}
 if(!in_array($ft7Type, $allowedFileType)){
  $uploadingError="Only 'jpg','jpeg','png','gif' formate of logo image are allowed";
}
 if(!in_array($ft8Type, $allowedFileType)){
  $uploadingError="Only 'jpg','jpeg','png','gif' formate of logo image are allowed";
}
...
// for empty file
 if(empty($uploadingError)){
   move_uploaded_file($temppr1,$pr1Path);
   move_uploaded_file($temppr2,$pr2Path);
   move_uploaded_file($temppr3,$pr3Path);
   move_uploaded_file($tempft1,$ft1Path);
   move_uploaded_file($tempft2,$ft2Path);
   move_uploaded_file($tempft3,$ft3Path);
   move_uploaded_file($tempft4,$ft4Path);
   move_uploaded_file($tempft5,$ft5Path);
   move_uploaded_file($tempft6,$ft6Path);
   move_uploaded_file($tempft7,$ft7Path);
   move_uploaded_file($tempft7,$ft8Path);
